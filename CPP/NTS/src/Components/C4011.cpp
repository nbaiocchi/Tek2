...
#include "../include/Components/C4011.hpp"
...
nts::Tristate C4011::compute(std::size_t pin)
{
    ...
    {
    case 3:
        _pins[3]->set_pin_value(Gates::NAND(this->compute(1), this->compute(2)), 3);
        return (_pins[3]->get_pin_value());
        break;
    case 4:
        _pins[4]->set_pin_value(Gates::NAND(this->compute(5), this->compute(6)), 4);
        return (_pins[4]->get_pin_value());
        break;
    case 10:

        _pins[10]->set_pin_value(Gates::NAND(this->compute(8), this->compute(9)), 10);
        return (_pins[10]->get_pin_value());
        break;
    case 11:
        _pins[11]->set_pin_value(Gates::NAND(this->compute(12), this->compute(13)), 11);
...
void C4011::setLink(std::size_t pin, IComponent &other, std::size_t otherPin)
{
    (void)pin;
    (void)other;
    (void)otherPin;
...
{    
    std::cout << "[Dump] C4011 : " << this->getName() << " \n";
...
std::shared_ptr<Pin> C4011::getPin(int index)
{
    return (this->_pins[index]);
}
void C4011::setName(std::string name)
{
    _name = name;
};
std::string C4011::getName() const
{
    return (_name);
};